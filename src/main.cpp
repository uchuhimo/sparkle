...
//            std::cout << x->data << std::endl;
    ...
    auto &&producer = sparkle::producer(actor_system)
            .OnRun(
                    [&consumer] {
                        for (int64_t i = 0L; i < TOTAL_ELEMENTS; ++i) {
                            consumer->Send(std::make_unique<Message>(i + 2));
                        }
                    })
            .Create();
    actor_system.Start();
...
void test_stateful_reactor() {
    sparkle::ActorSystem actor_system;
    auto &&stateful_reactor = sparkle::reactor<int32_t, NoDefaultConstructor>(
            actor_system).Create();
}
    ...
    auto &&producer = sparkle::producer<int32_t>(actor_system)
            .OnRun(
                    [&consumer](int32_t &state) {
                        for (int64_t i = 0L; i < TOTAL_ELEMENTS; ++i) {
                            consumer->Send(std::make_unique<Message>(i));
                        }
                    })
            .Create();
    actor_system.Start();
            ...
            .OnRun(
                    [&consumers] {
                        for (int64_t i = 0L; i < 6; ++i) {
                            consumers.Get(i % consumers.size())->Send(std::make_unique<Message>(i));
                        }
                    })
            .CreateGroup(4);
