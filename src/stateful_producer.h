 ...
 public:
  ...
  class Builder : public Group<StatefulProducer<S>>::template Builder<Builder> {
   public:
    ...
    Builder(ActorSystem &actor_system) : BaseBuilder(actor_system), on_setup_([](S &, const Context &) {}), on_shutdown_([](S &, const Context &) {}), on_run_([](S &, const Context &) {}) {}
    ...
    Builder OnRun(const std::function<void(S &)> &on_run) {
      return OnRunWithContext([on_run](S &state, const Context &context) { on_run(state); });
    }

    std::shared_ptr<StatefulProducer> CreateWithContext(const Context &context) {
      auto producer = std::make_shared<StatefulProducer>(StatefulProducer(context, on_setup_, on_shutdown_, on_run_));
