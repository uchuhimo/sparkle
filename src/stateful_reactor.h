 ...
 public:
  ...
  class Builder : public Group<StatefulReactor<T, S>>::template Builder<Builder> {
   public:
    ...
    Builder(ActorSystem &actor_system)
        : BaseBuilder(actor_system), mailbox_size_{0}, on_setup_([](S &, const Context &) {}),
          on_shutdown_([](S &, const Context &) {}), on_receive_([](T &, S &, const Context &) {}) {}
    ...
    Builder OnSetup(const std::function<void(S &)> &on_setup) {
      return OnSetupWithContext([on_setup](S &state, const Context &context) { on_setup(state); });
    }

    Builder OnShutdownWithContext(const std::function<void(S &, const Context &)> &on_shutdown) {
      on_shutdown_ = on_shutdown;
      return *this;
    }

    Builder OnShutdown(const std::function<void(S &)> &on_shutdown) {
      return OnShutdownWithContext([on_shutdown](S &state, const Context &context) { on_shutdown(state); });
    }

    Builder OnReceiveWithContext(const std::function<void(T &, S &, const Context &)> &on_receive) {
    ...
    Builder OnReceive(const std::function<void(T &, S &)> &on_receive) {
      return OnReceiveWithContext(
          [on_receive](T &message, S &state, const Context &context) { on_receive(message, state); });
    }
    ...
    std::shared_ptr<StatefulReactor> CreateWithContext(const Context &context) {
      ...
      auto reactor = std::make_shared<StatefulReactor>(
          StatefulReactor(context, mailbox_size_, on_setup_, on_shutdown_, on_receive_));
  ...
  StatefulReactor(const Context &actor_context,
      ...
      : Reactor<T>(actor_context,
                   ...
                   [this, on_shutdown](const Context &context) { on_shutdown(state_, context); },
                   [this, on_receive](T &message, const Context &context) { on_receive(message, state_, context); }),
        state_(S()) {
  }
