 ...
 public:
  ...
  class Builder : public Group<Reactor<T>>::template Builder<Builder> {
   public:
    ...
    Builder(ActorSystem &actor_system) : BaseBuilder(actor_system), mailbox_size_{0}, on_setup_([](const Context &) {}),
                on_shutdown_([](const Context &) {}), on_receive_([](T &, const Context &) {}) {}
    ...
    Builder OnReceiveWithContext(const std::function<void(T &, const Context &)> &on_receive) {
      on_receive_ = on_receive;
      return *this;
    }

    Builder OnReceive(const std::function<void(T &)> &on_receive) {
      return OnReceiveWithContext([on_receive](T &message, const Context &) { on_receive(message); });
    }
  ...
  Reactor(const Context &context,
      ...
      : Actor(context, on_setup, on_shutdown), mailbox_(mailbox_size), on_receive_(on_receive) {}
